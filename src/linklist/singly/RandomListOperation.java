...
public class RandomListOperation {
    ...
    /**
     * This solution works in leetcode, previous one failing in some edge cases.
     * @param head
     * @return
     */
    public RLNode copyRandomList(RLNode head) {
        if (head == null) return null;

        HashMap<RLNode, RLNode> oldToNew = new HashMap<>();

        RLNode curr = head;
        while (curr != null) {
            oldToNew.put(curr, new RLNode(curr.data));
            curr = curr.next;
        }
